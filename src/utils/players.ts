...
/**
 * İlgili başlıkları ve kaynak URL'leri ile birlikte film oynatıcılarının bir listesini oluşturur.
 * Her oynatıcı, verilen film kimliği kullanılarak oluşturulur.
 *
 * @param {string | number} id - Oynatıcı URL'lerine yerleştirilecek filmin kimliği.
 * @param {number} [startAt] - Oynatıcı URL'lerine yerleştirilecek saniye cinsinden başlangıç ​​konumu. İsteğe bağlı.
 * @returns {PlayersProps[]} - Her biri oynatıcının başlığını ve ilgili kaynak URL'sini içeren nesnelerden oluşan bir dizi.
 */
export const getMoviePlayers = (id: string | number, startAt?: number): PlayersProps[] => {
  return [
    {
      title: "VidKing",
      source: `https://www.vidking.net/embed/movie/${id}?progress=${startAt}`,
      recommended: true,
      fast: true,
      ads: false,
      resumable: true,
...
export const getTvShowPlayers = (
  ...
  return [
    {
      title: "VidKing",
      source: `https://www.vidking.net/embed/tv/${id}/${season}/${episode}?progress=${startAt || ""}&color=eefdec`,
      fast: true,
      ads: false,
      resumable: true,
    },
