...
export type VidkingEventType = "play" | "pause" | "seeked" | "ended" | "timeupdate";

export interface VidkingEventData {
  type: "PLAYER_EVENT";
  data: {
    event: VidkingEventType;
    currentTime: number;
    duration: number;
    mtmdbId: number;
...
type Data = VidkingEventData["data"];

export interface UseVidkingPlayerOptions {
  metadata?: {
    season?: number;
    episode?: number;
  };
...
export function useVidkingPlayer(options: UseVidkingPlayerOptions = {}) {
  ...
  const [lastEvent, setLastEvent] = useState<VidkingEventType | null>(null);
  ...
  const syncToServer = async (data: Data, completed?: boolean) => {
    ...
    if (diff(data.currentTime, lastCurrentTime) <= 5) return; //spam'ı önle
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      const data = event.data as VidkingEventData;
