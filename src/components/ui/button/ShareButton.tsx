...
const HASTAGS = ["Yabancı Dizi", "free", "movies", "streaming"];
...
const ShareButton: React.FC<ShareButtonProps> = ({ title, id, type = "movie" }) => {
  ...
  const description = `Check out and stream ${title} on Yabancı Dizi for FREE!!`;

  const [opened, { open, close }] = useDisclosure(false);

  return (
    <>
      <IconButton icon={<Share size={20} />} variant="ghost" tooltip="Paylaş" onPress={open} />
      <VaulDrawer open={opened} onClose={close} backdrop="blur" title="Share via">
        ...
        <div className="space-y-4 px-6">
          <CopyButton text={url} label="Kopyala" copiedLabel="Link kopyalandı!" onCopied={close} />
        </div>
      </VaulDrawer>
    </>
  );
