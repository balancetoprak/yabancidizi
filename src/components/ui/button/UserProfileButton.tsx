...
const UserProfileButton: React.FC = () => {
  ...
  const ITEMS: DropdownItemProps[] = useMemo(
    () => [
      // TODO: admin sayfası eklenecek
      // { label: "admin", href: "/manager", icon: <Gear /> },
      {
        label: "Çıkış Yap",
        onClick: async () => {
          if (logout) return;
  ...
  const guest = !user;
  const avatar = `${env.NEXT_PUBLIC_AVATAR_PROVIDER_URL}${user?.avatar}`;

  const ProfileButton = (
    <Button
      title={guest ? "Giriş Yap" : user.username}
      variant="light"
