...
const requirements = [
  { re: /[0-9]/, label: "Numarayı içerir" },
  { re: /[a-z]/, label: "Küçük harf içerir" },
  { re: /[A-Z]/, label: "Büyük harf içerir" },
  { re: /[$&+,:;=?@#|'<>.^*()%!-]/, label: "Özel sembol içerir" },
];
...
const PasswordRequirement = memo(({ meets, label }: { meets: boolean; label: string }) => {
  return (
    <p className={`text-small mt-1.5 flex items-center ${meets ? "text-success" : "text-danger"}`}>
    ...
    return (
      ...
      >
        ...
        {meter && withStrengthMeter && (
          <div
            className={cn(
              "rounded-medium border-foreground-200 bg-secondary-background absolute z-100 w-full border-2 p-4 shadow-lg",
              {
