...
const TvShowEpisodesSelection: React.FC<TvShowEpisodesSelectionProps> = ({
      ...
      const episodeDetails = await Promise.all(
        season.episodes.map(async (ep) => {
          const res = await fetch(
            `https://api.themoviedb.org/3/tv/${id}/season/${seasonNumber}/episode/${ep.episode_number}?append_to_response=translations&language=tr-TR`,
            {
              headers: {
          ...
          const trTranslation = episodeData.translations.translations.find(
            (t: any) => t.iso_639_1 === "tr",
          );

          return {
            ...ep,
            name: trTranslation?.data?.name || ep.name,
            overview: trTranslation?.data?.overview || ep.overview,
          };
        }),
      );

      return { ...season, episodes: episodeDetails };
    },
    queryKey: ["tv-show-episodes", id, seasonNumber, "tr"],
  });
  ...
  if (isEmpty(EPISODES)) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-center">Bölüm Bulunamadı</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {isNotReleased ? "Yayınlanmadı" : movieDurationString(episode.runtime)}
        </Chip>
        <Chip
          size="sm"
      ...
      <CardBody className="flex space-y-1">
        ...
        <p className="text-content4-foreground line-clamp-1 text-xs">
          {formatDate(episode.air_date, "tr-TR")}
        </p>
