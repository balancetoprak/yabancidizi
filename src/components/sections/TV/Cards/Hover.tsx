...
const TvShowHoverCard: React.FC<{ id: number; fullWidth?: boolean }> = ({ id, fullWidth }) => {
  const { data: tv, isPending } = useQuery({
    queryFn: () => tmdb.tvShows.details(id, ["images"], "tr-TR"),
    queryKey: ["get-tv-detail-on-hover-poster", id, "tr-TR"],
  });
    ...
    >
      <div className="relative">
        <div className="absolute aspect-video h-fit w-full">
          <div className="from-secondary-background absolute z-2 h-full w-full bg-linear-to-t from-1%"></div>
          {!isEmpty(titleImage) && (
            <Image
              isBlurred
              radius="none"
        ...
        <div className="flex flex-col gap-2 p-4 pt-[40%] *:z-10">
          ...
          <div className="md:text-md flex flex-wrap gap-1 text-xs md:gap-2">
            <div className="flex items-center gap-1">
              <Season />
              <span>{tv.number_of_seasons} Sezon</span>
            </div>
            <p>&#8226;</p>
            <div className="flex items-center gap-1">
              <List />
              <span>{tv.number_of_episodes} Bölüm</span>
            </div>
            <p>&#8226;</p>
            <div className="flex items-center gap-1">
              <Calendar />
