...
const SORT_OPTIONS: { key: SortOption; label: string }[] = [
  ...
  { key: "release_date", label: "Yayınlanma Tarihi" },
...
const LibraryList = () => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status, refetch } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam = 1 }) => {
        ...
        return await getWatchlist("all" as FilterType, pageParam, ITEMS_PER_PAGE);
      },
      initialPageParam: 1,
  ...
  const clearWatchlistMutation = useMutation({
    mutationFn: async (type: "movie" | "tv") => {
      ...
      if (!result.success) {
        throw new Error(result.error || "Fsiled");
      }
    ...
    onSuccess: ({ type, count }) => {
      ...
      addToast({
        title: `${count} ${type === "movie" ? "Film" : "dizi"} listenize eklendi!`,
        color: "success",
        icon: <Trash />,
      });
    ...
    onError: (error) => {
      addToast({
        title: "Error",
        description: "Listenize eklenirken sorun oluştu!",
        color: "danger",
      });
  ...
  if (status === "error") {
    return (
      <div className="flex h-[50vh] flex-col items-center justify-center gap-4">
        <p className="text-danger">Liste yüklenirken hata oluştu</p>
